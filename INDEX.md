...
| 2026-08-10 | AI CEO (prime agent) | Implemented spec 017 P2 (#50). The governed wiki write-back landed (AC4, AC5). Three endpoints: `POST .../wiki/annotations`, `POST .../wiki/pages/facts/{fact_id}/edits`, and `POST .../wiki/answers`. Annotations land in the `wiki/annotations` namespace. Filed answers land in the `derived` namespace (provenance kind derived, 011 R5). A write without a registered policy fails closed with `write-policy-rejected`. The conformance suite is green. |
