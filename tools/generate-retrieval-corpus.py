    ...
    if category == "exact-name":
        ...
        common["facts"] = [
            fact(
                ...
                embedding_fixture_role="near",
            ),
            fact(
                sid,
                "distractor",
                namespace=namespace,
                key=f"other-{index:03d}",
                text=f"{token} {token}",
                embedding_fixture_role="distractor",
            ),
        ]
        ...
        common["facts"] = [
            ...
            fact(
                ...
                namespace=namespace,
                key=f"state-{index:03d}",
                text=f"{token} corrected evidence",
                embedding_fixture_role="relevant",
        ...
        common["expected_candidate_ids"] = (
            [f"{sid}-root"] if index % 2 else [f"{sid}-relevant", f"{sid}-distractor"]
        )
        ...
        common["facts"] = [
            fact(
                ...
                "relevant",
                namespace=namespace,
                key=f"recent-{index:03d}",
                text=f"{token} current",
                embedding_fixture_role="relevant",
                observed_at=BASE_VALID,
                valid_from=BASE_VALID,
                write_policy_id="temporal-active-case-evidence",
            ),
            fact(
                sid,
        ...
        common["facts"] = [
            fact(
                ...
                "relevant",
                namespace=namespace,
                key=f"stable-{index:03d}",
                text=f"{token} durable",
                embedding_fixture_role="relevant",
                observed_at="2026-03-01T00:00:00Z",
                valid_from="2026-03-01T00:00:00Z",
            ),
        ...
        common["facts"] = [
            fact(
                ...
                "trap",
                namespace=namespace,
                key=f"conflict-{index:03d}",
                text=f"{token} unresolved alternatives",
                embedding_fixture_role="trap",
                sensitivity="restricted",
            ),
        ]
    elif category == "isolation-lifecycle":
        trap_group = (index - 1) // 8
        trap_scope = ["secondary-tenant", "secondary-subject", "primary", "primary"][
            trap_group
        ]
        trap_sensitivity = "restricted" if trap_group in (0, 2) else "internal"
        lifecycle = (
            "active" if trap_group < 3 else ("deleted" if index <= 28 else "expired")
        )
        common["forbidden_ids"] = [f"{sid}-trap"]
        common["facts"] = [
            fact(
                ...
                "relevant",
                namespace=namespace,
                key=f"allowed-{index:03d}",
                text=f"{token} authorized",
                embedding_fixture_role="relevant",
            ),
            fact(
                sid,
                ...
                scope=trap_scope,
                namespace=namespace,
                key=f"trap-{index:03d}",
                text=f"{token} {token} private trap",
                embedding_fixture_role="trap",
                sensitivity=trap_sensitivity,
...
def main() -> None:
    ...
    manifest = {
        ...
        "embedder": {
            "id": "embedding-conformance-4d-v1",
            "version": "1",
