...
test("remember uses separate governed episode and fact writes", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      "019be000-0000-7000-8000-000000000040",
    );
    ...
    assert.equal(
      JSON.parse(requests[0].init.body).payload.content,
      "The address is 10 Test Street.",
    );
    assert.equal(
      JSON.parse(requests[1].init.body).evidence_episode_ids[0],
      "019be000-0000-7000-8000-000000000040",
    );
  } finally {
...
test("consolidation writes only validated episode lineage", async () => {
  ...
  globalThis.fetch = async (url, init) => {
    ...
    return new Response(
      JSON.stringify({ fact_id: "019be000-0000-7000-8000-000000000050" }),
      {
        status: 201,
  ...
  try {
    ...
    assert.deepEqual(result.source_episode_ids, [
      PROJECT_EPISODE_A,
      PROJECT_EPISODE_B,
    ]);
    assert.equal(
      result.writes[0].fact.fact_id,
      "019be000-0000-7000-8000-000000000050",
    );
    assert.deepEqual(requests[0].body.evidence_episode_ids, [
      PROJECT_EPISODE_A,
      PROJECT_EPISODE_B,
    ]);
    assert.match(
    ...
    await assert.rejects(
      client.consolidateProjectReview(
        ...
        [
          consolidationWrite("release-target-difference"),
          consolidationWrite(
            "release-target-difference-copy",
            "claim-release-target-copy",
          ),
        ],
...
test("partial remember preserves the committed episode and typed HTTP cause", async () => {
  ...
  globalThis.fetch = async () => {
    ...
    if (calls === 1) {
      return new Response(
        JSON.stringify({ episode_id: "019be000-0000-7000-8000-000000000040" }),
        {
          status: 201,
          headers: { "content-type": "application/json" },
        },
  ...
  try {
    ...
    await assert.rejects(
      client.remember("Keep the evidence.", {
        key: "partial",
...
test("export ready redirects remain visible and are not followed", async () => {
  ...
  try {
    ...
    const response = await client.getExportResponse(
      "019be000-0000-7000-8000-0000000000c0",
      {
...
test("recallByProject keeps each project candidate set separate", async () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(result), [
      ...
      "project-bbbbbbbbbbbbbbbb",
    ]);
    assert.deepEqual(
      requests.map(({ body }) => body.filters),
      [
...
test("compareProjectBundles marks key differences without semantic overclaim", () => {
  ...
  const groups = Object.fromEntries(
    comparison.groups.map((group) => [group.key, group]),
  );
  assert.equal(
    groups["release-target"].classification,
...
test("compareProjectBundles recognizes exact canonical values", () => {
  const comparison = compareProjectBundles({
    "project-a": {
      items: [
        { key: "same", value: { content: "v1", metadata: { source: "a" } } },
      ],
    },
    "project-b": {
      items: [
        { key: "same", value: { metadata: { source: "a" }, content: "v1" } },
      ],
    },
  });
...
test("compareProjectBundles returns bounded lexical review candidates", () => {
  ...
  assert.deepEqual(
    comparison.lexical_review.candidates[0].token_delta.only_in,
    {
      "project-a": ["stable"],
      "project-b": ["beta"],
    },
  ...
  assert.ok(
    comparison.lexical_review.candidates[0].token_delta.shared.includes(
      "release",
    ),
  );
  assert.equal(
    comparison.lexical_review.candidates[0].token_delta.truncated,
    false,
  );
  assert.equal(comparison.semantic_inference.performed, false);
});
...
test("compareProjectBundles exposes observed project context", () => {
  const comparison = compareProjectBundles({
    ...
    "project-b": {
      items: [{ key: "decision-c", value: { content: "release target" } }],
    },
  });
